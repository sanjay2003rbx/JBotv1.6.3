...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>stats' then
		message.channel:sendMessage(string.format(' ``` Name: JBot#7214. Bot: Yes. Desc: Multipurpose bot used to moderate servers or provide fun commands for you and your server. Language: Lua. Framework: Luvit. Lib: Discordia. Lib Creator: Sinister Rectus. Region: World. Version: 1.6.3. Bot Owner: Sanjay#2382 Updated: Yes. Updates: No. ```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '>new' then
		message.channel:sendMessage(string.format(' ``` Current Version 1.6.3: Added >quote for your favorite quotes and >joke for a great laugh! Both are randomized now. :) ```, %s', message.author:getMentionString()))
